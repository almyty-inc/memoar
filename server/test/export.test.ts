...
describe("session export", () => {
  it("exports a canonical bundle that round-trips through the canonical-bundle parser", async () => {
    ...
    const exported = await service.export(TEST_CONTEXT, TEST_SESSION.id, "canonical");
    expect(exported.contentType).toBe("application/json");
    expect(exported.filename).toBe(`${TEST_SESSION.id}.memoar.json`);

    const seed = Object.fromEntries(Object.entries(canonicalProjection(TEST_SESSION)).filter(([key]) => key !== "turns"));
    ...
    expect(parsed.sessions[0]).toEqual(canonicalProjection(TEST_SESSION));
  });
  ...
  it("exports self-contained escaped HTML", async () => {
    ...
    const exported = await service.export(TEST_CONTEXT, session.id, "html");
  ...
  it("rejects unknown formats and missing sessions", async () => {
    ...
    await store.saveSession(TEST_CONTEXT, TEST_SESSION);
    const service = new SessionsService(store);
    await expect(service.export(TEST_CONTEXT, TEST_SESSION.id, "pdf")).rejects.toMatchObject({ response: { code: "invalid_export_format" } });
    await expect(service.export(TEST_CONTEXT, "0191cafe-0000-7000-8000-00000000dead", "canonical")).rejects.toThrow("Session not found");
  });
});
