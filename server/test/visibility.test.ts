...
describe("session visibility PATCH", () => {
  it("rejects widening beyond private without a completed redaction review", async () => {
    ...
    await expect(sharing.updateVisibility(TEST_CONTEXT, TEST_SESSION.id, { visibility: { scope: "team", teamId: TEAM_ID } }))
    ...
    expect(session!.visibility.scope).toBe(TEST_SESSION.visibility.scope);
  });
  ...
  it("widens with a current review, preserves the owner, and narrows back without one", async () => {
    ...
    expect(narrowed.visibility).toEqual({ scope: "private", ownerId: TEST_SESSION.visibility.ownerId });
  });
  ...
  it("rejects widening when the session content changed after the review", async () => {
    ...
    await store.saveSession(TEST_CONTEXT, session);
    await expect(sharing.updateVisibility(TEST_CONTEXT, session.id, {
  ...
  it("404s for a missing session", async () => {
    ...
    await expect(sharing.updateVisibility(TEST_CONTEXT, "0191cafe-0000-7000-8000-00000000dead", { visibility: { scope: "private" } }))
