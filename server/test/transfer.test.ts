...
describe("direct transfer", () => {
  it("copies the canonical session and provenance into the recipient tenant on acceptance", async () => {
    ...
    expect(copiedId).not.toBe(DEMO_SESSION.id);
    ...
    expect(copied!.turns).toHaveLength(DEMO_SESSION.turns.length);
    expect(copied!.turns.map((turn) => turn.id)).not.toEqual(DEMO_SESSION.turns.map((turn) => turn.id));
    expect(copied!.turns[1]!.parentId).toBe(copied!.turns[0]!.id);
    expect(copied!.turns[0]!.blocks[0]!.text).toBe(DEMO_SESSION.turns[0]!.blocks[0]!.text);
    const imported = copied!.provenance.at(-1)!;
    expect(imported.kind).toBe("import");
    expect(imported.sourceId).toBe(`transfer:${transfer.id}:${DEMO_SESSION.id}`);

    expect(await store.getSession(recipient, DEMO_SESSION.id)).toBeNull();
