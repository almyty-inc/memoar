...
async function seededMachine(store: DevArchiveStore): Promise<MachinesService> {
  await store.saveMachine(TEST_CONTEXT, {
    id: MACHINE_ID, tenantId: TEST_CONTEXT.tenantId, name: "laptop", platform: "darwin",
...
describe("durable machine commands", () => {
  it("replays unacked commands on connect, marks them delivered, and pushes new ones", async () => {
    ...
    const first = await store.createMachineCommand(TEST_CONTEXT, { machineId: MACHINE_ID, kind: "materialize", payload: { jobId: "job-1" } });

    const events = await (async () => {
      const promise = collectEvents(service, TEST_CONTEXT, 120);
      setTimeout(() => {
        void store.createMachineCommand(TEST_CONTEXT, { machineId: MACHINE_ID, kind: "materialize", payload: { jobId: "job-2" } });
      }, 40);
      return promise;
    })();
    ...
    const unacked = await store.listUnackedMachineCommands(TEST_CONTEXT, MACHINE_ID);
  ...
  it("replays delivered-but-unacked commands on reconnect and drops them after ack", async () => {
    const store = new DevArchiveStore();
    const service = await seededMachine(store);
    const command = await store.createMachineCommand(TEST_CONTEXT, { machineId: MACHINE_ID, kind: "materialize", payload: { jobId: "job-1" } });

    expect(commandEvents(await collectEvents(service, TEST_CONTEXT, 60))).toHaveLength(1);
    expect(commandEvents(await collectEvents(service, TEST_CONTEXT, 60))).toHaveLength(1);

    await service.ackCommand(TEST_CONTEXT, MACHINE_ID, command.id, { status: "completed" });
    expect(commandEvents(await collectEvents(service, TEST_CONTEXT, 60))).toHaveLength(0);
    expect(await store.listUnackedMachineCommands(TEST_CONTEXT, MACHINE_ID)).toHaveLength(0);
  });

  it("records failed acks with their error and 404s unknown commands", async () => {
    const store = new DevArchiveStore();
    const service = await seededMachine(store);
    const command = await store.createMachineCommand(TEST_CONTEXT, { machineId: MACHINE_ID, kind: "materialize", payload: {} });

    await service.ackCommand(TEST_CONTEXT, MACHINE_ID, command.id, { status: "failed", error: "target store locked" });
    expect(await store.listUnackedMachineCommands(TEST_CONTEXT, MACHINE_ID)).toHaveLength(0);

    await expect(service.ackCommand(TEST_CONTEXT, MACHINE_ID, "0191cafe-0000-7000-8000-00000000dead", { status: "completed" })).rejects.toThrow("Command not found");
  });

  it("rejects machine tokens bound to a different machine for stream and ack", async () => {
    const store = new DevArchiveStore();
    const service = await seededMachine(store);
    const other: TenantContext = { ...TEST_CONTEXT, machineId: "0191cafe-0000-7000-8000-0000000000ff" };
  ...
  it("errors the stream for an unknown machine", async () => {
    ...
    await expect(collectEvents(service, TEST_CONTEXT, 60)).rejects.toThrow("Machine not found");
  });
});
